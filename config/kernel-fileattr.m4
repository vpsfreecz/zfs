...
dnl #
dnl # Linux 6.18 API change,
dnl # inode_operations->fileattr_{get,set}() switched from struct fileattr *
dnl # to struct file_kattr *.
dnl #
AC_DEFUN([ZFS_AC_KERNEL_SRC_FILEATTR_OPS], [
	ZFS_LINUX_TEST_SRC([inode_operations_filekattr_idmap], [
		#include <linux/fs.h>
		...
		char fileattr_set_match[
		    ...
		    struct file_kattr *)) ? 1 : -1] __attribute__ ((unused));
		char fileattr_get_match[
		    __builtin_types_compatible_p(fileattr_get_t,
		    int (*)(struct dentry *, struct file_kattr *)) ? 1 : -1]
		    __attribute__ ((unused));
	],[])

	ZFS_LINUX_TEST_SRC([inode_operations_filekattr_userns], [
		#include <linux/fs.h>
		#include <linux/fileattr.h>
...
AC_DEFUN([ZFS_AC_KERNEL_FILEATTR_OPS], [
	...
	ZFS_LINUX_TEST_RESULT([inode_operations_filekattr_idmap], [
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_FILEATTR_OPS, 1,
		    [inode_operations has fileattr_get/fileattr_set callbacks])
		AC_DEFINE(HAVE_FILE_KATTR, 1,
		    [inode_operations fileattr callbacks use struct file_kattr])
	],[
		ZFS_LINUX_TEST_RESULT([inode_operations_filekattr_userns], [
			AC_MSG_RESULT(yes)
			AC_DEFINE(HAVE_FILEATTR_OPS, 1,
			    [inode_operations has fileattr_get/fileattr_set callbacks])
			AC_DEFINE(HAVE_FILE_KATTR, 1,
