...
static int
zfs_read_folio_range(objset_t *os, uint64_t object, u_offset_t file_off,
    struct folio *folio, size_t folio_off, size_t len, int flags)
{
	size_t copied = 0;
	...
	while (copied < len) {
		...
		error = dmu_read(os, object, file_off + copied, nbytes,
		    (char *)va + page_off, flags);
		kunmap(pp);
		if (error != 0)
			return (error);
...
static void
zfs_write_folio_range(objset_t *os, uint64_t object, u_offset_t file_off,
    struct folio *folio, size_t folio_off, size_t len, dmu_tx_t *tx)
{
	size_t copied = 0;
...
static void
zfs_zero_folio_range(struct folio *folio, size_t off, size_t len)
{
	size_t cleared = 0;
 ...
 * cache pages win over the DMU view.  The caller holds the matching ZFS range
 * lock, which also excludes page_mkwrite() while each slice is captured.
 */
int
 ...
 * under a slice-local ZFS range lock into a bounce buffer, then drop that lock
 * before copying the stable snapshot to the caller.  User-buffer faults
 * therefore cannot deadlock against page_mkwrite(), truncate, or hole-punch.
 */
int
mappedread_writable(znode_t *zp, int nbytes, zfs_uio_t *uio)
{
	...
	for (start &= PAGE_MASK; len > 0; start += PAGE_SIZE) {
		...
		uint64_t file_off = start + off;
		zfs_locked_range_t *lr;

		lr = zfs_rangelock_enter(&zp->z_rangelock, file_off, bytes,
		    RL_READER);
		if (file_off >= zp->z_size) {
			zfs_rangelock_exit(lr);
			break;
		}
		bytes = MIN(bytes, zp->z_size - file_off);

		error = zfs_read_mapped_range(zp, file_off, bytes, buf,
		    DMU_READ_PREFETCH);
		zfs_rangelock_exit(lr);
		if (error != 0)
			goto out;
...
zfs_setattr_dir(znode_t *dzp)
{
	...
	dmu_tx_t	*tx = NULL;
	uint64_t	uid, gid;
	boolean_t	update_uid, update_gid;
	...
	while ((err = zap_cursor_retrieve(&zc, zap)) == 0) {
		...
		xip = ZTOI(zp);
		update_uid = KUID_TO_SUID(xip->i_uid) !=
		    KUID_TO_SUID(dxip->i_uid);
		update_gid = KGID_TO_SGID(xip->i_gid) !=
		    KGID_TO_SGID(dxip->i_gid);
		if (!update_uid && !update_gid &&
		    zp->z_projid == dzp->z_projid)
			goto next;
		if (update_uid && (err = zfs_ugid_map_host_to_ns(
		    zfsvfs->z_uid_map, KUID_TO_SUID(dxip->i_uid), &uid)) != 0)
			break;
		if (update_gid && (err = zfs_ugid_map_host_to_ns(
		    zfsvfs->z_gid_map, KGID_TO_SGID(dxip->i_gid), &gid)) != 0)
			break;
...
zfs_setattr(znode_t *zp, vattr_t *vap, int flags, cred_t *cr, zidmap_t *mnt_ns)
{
	...
	int		trim_mask = 0;
	uint64_t	new_mode;
	uint64_t	new_kuid = 0, new_kgid = 0;
...
top:
	...
	if ((mask & (ATTR_UID | ATTR_GID)) || projid != ZFS_INVALID_PROJID) {
		...
		if (mask & ATTR_UID) {
			err = zfs_ugid_map_host_to_ns(zfsvfs->z_uid_map,
			    new_kuid, &stored_uid);
			if (err == 0)
				err = zfs_ugid_map_ns_to_host(zfsvfs->z_uid_map,
				    stored_uid, &offset_uid);
		}
		if (err == 0 && (mask & ATTR_GID)) {
			err = zfs_ugid_map_host_to_ns(zfsvfs->z_gid_map,
			    new_kgid, &stored_gid);
			if (err == 0)
				err = zfs_ugid_map_ns_to_host(zfsvfs->z_gid_map,
				    stored_gid, &offset_gid);
		}
		if (err != 0) {
			if (attrzp)
				zrele(attrzp);
	...
	if (mask & (ATTR_UID|ATTR_GID)) {
		...
		if (mask & ATTR_UID) {
			...
			if (attrzp) {
				SA_ADD_BULK_ATTR(xattr_bulk, xattr_count,
				    SA_ZPL_UID(zfsvfs), NULL, &stored_uid,
				    sizeof (stored_uid));
				ZTOI(attrzp)->i_uid = SUID_TO_KUID(offset_uid);
			}
		}
		...
		if (mask & ATTR_GID) {
			...
			if (attrzp) {
				SA_ADD_BULK_ATTR(xattr_bulk, xattr_count,
				    SA_ZPL_GID(zfsvfs), NULL, &stored_gid,
				    sizeof (stored_gid));
				ZTOI(attrzp)->i_gid = SGID_TO_KGID(offset_gid);
			}
		}
		if (!(mask & ATTR_MODE)) {
	...
	if (error == 0) {
		...
		if (error == 0 && szp->z_is_tmpfile &&
			...
			txg_wait_flag_t wait_flags =
			    ...
			    ZIO_FAILURE_MODE_CONTINUE ? TXG_WAIT_SUSPEND :
			    TXG_WAIT_NONE;
...
/*
 * Push a page-cache folio out to disk. Once the cache unit is on stable
 * storage the registered commit callback will be run as notification of
 * completion.
 *
 *	IN:	ip	    - folio mapped for inode.
...
range_retry:
	...
	zfs_write_folio_range(zfsvfs->z_os, zp->z_id, pgoff, folio,
	    pgoff - folio_pos(folio), pglen, tx);
