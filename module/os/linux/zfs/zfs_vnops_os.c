 ...
 * under the caller's ZFS range lock into a bounce buffer, then copy that
 * stable snapshot to the caller without holding page locks so user-buffer
 * faults cannot deadlock against page_mkwrite(), truncate, or hole-punch.
 */
static int
