...
top:
	...
	if (mask & (ATTR_UID|ATTR_GID)) {
		...
		if (mask & ATTR_UID) {
			stored_uid = zfs_ugid_map_host_to_ns(
			    zfsvfs->z_uid_map, new_kuid);
			offset_uid = zfs_ugid_map_ns_to_host(
			    zfsvfs->z_uid_map, new_kuid);
		...
		if (mask & ATTR_GID) {
			stored_gid = zfs_ugid_map_host_to_ns(
			    zfsvfs->z_gid_map, new_kgid);
			offset_gid = zfs_ugid_map_ns_to_host(
			    zfsvfs->z_gid_map, new_kgid);
    ...
    offset_t offset, cred_t *cr, zidmap_t *mnt_ns)
{
	 ...
	 * On Linux we can reach this path both from VFS entry points
	 * that already know the caller's mount idmap and from internal
	 * callers like replay.  Keep the access check on the same mount
	 * idmap as the VFS walk when one exists, and fall back to
	 * init_idmap for internal paths.
	 */
