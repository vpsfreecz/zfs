...
#if defined(__linux__)
	/*
	 * A user copy can fault a writable mapping of this file and enter
	 * page_mkwrite().  Snapshot each slice under its own range lock, then
	 * copy it after dropping that lock to avoid recursively waiting for the
	 * request-wide reader lock.
	 */
	if ((uio->uio_extflg & UIO_DIRECT) == 0 && zn_writably_mapped(zp)) {
		zfs_rangelock_exit(lr);
		lr = NULL;
		error = mappedread_writable(zp, n, uio);
		if (error == ECKSUM)
			error = SET_ERROR(EIO);
		if (error == EFAULT &&
    ...
    struct lwb *lwb, zio_t *zio)
{
	...
	if (buf != NULL) { /* immediate write */
		...
		if (error == 0) {
			...
			blkptr_t direct_bp = { 0 };
			boolean_t direct_write;

			mutex_enter(&db->db_mtx);
			direct_write = dmu_buf_get_diowrite_bp_copy_locked(db,
			    lr->lr_common.lrc_txg, &direct_bp);
		...
		if (error != 0) {
			...
			if (error == EAGAIN && zfs_bclone_wait_dirty) {
				txg_wait_flag_t wait_flags =
				    ...
				    TXG_WAIT_SUSPEND : TXG_WAIT_NONE;
