	...
	ddt_phys_variant_t v = DDT_PHYS_VARIANT(ddt, p);
		...
		if (ddt_phys_is_gang(dde->dde_phys, v)) {
			for (int i = 0; i < BP_GET_NDVAS(zio->io_bp); i++) {
				...
				/*
				 * OpenZFS master uses a newer metaslab_group_alloc_decrement()
				 * signature which tracks allocations by physical size. 2.3.x
				 * uses a tag-based refcount, so just drop our allocation tag
				 * here before retrying the write.
				 */
